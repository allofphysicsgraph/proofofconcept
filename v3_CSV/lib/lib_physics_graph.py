...
def convert_feed_csv_to_list_of_dics(feedDB):
    ...
    with open(feedDB, 'rb') as csvfile:
        ...
        for row in reader:
            this_line_dic = {}
            if(len(row) == 1 or len(row) == 0):  # skip empty lines
...
def set_of_infrule_from_list_of_dics(connections_list_of_dics):
    ...
    for connection_dic in connections_list_of_dics:
        if (connection_dic['from type'] == 'infrule'):
            list_of_infrule.append(
                str(connection_dic['from perm index'] + ":" + connection_dic['from temp index']))
        if (connection_dic['to type'] == 'infrule'):
            list_of_infrule.append(
                str(connection_dic['to perm index'] + ":" + connection_dic['to temp index']))
...
def convert_graphviz_to_pictures_with_and_without_labels(
        path_to_gv, extension, path_to_picture):
...
# os.system("convert "+folder_name+"/"+file_name+extension+"
# "+folder_name+"/"+file_name+extension) # for some reason the initial svg
# format isn't interperatable by graphviz. Thus, I pass the .svg through
# convert
...
def check_connection_DB_steps_for_single_inf_rule(connections_list_of_dics):
    # check that each step in the connections database only has one inference
    # rule
    list_of_deriv = get_set_of_derivations(connections_list_of_dics)
# print(list_of_deriv)
    for this_deriv in list_of_deriv:
        #   print(this_deriv)
        this_connections_list_of_dics = keep_only_this_derivation(
            this_deriv, connections_list_of_dics)
...
#   print(list_of_steps)
        for this_step in list_of_steps:
            ...
            if (len(set(infrule_list)) != 1):
                print(
                    "ERROR found: more than 1 infrule in " +
                    this_deriv +
                    " step " +
                    this_step)
    return
