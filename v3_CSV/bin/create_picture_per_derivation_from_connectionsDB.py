...
graphviz_filename = output_path + 'connections_' + \
    which_derivation_to_make_no_spaces + '.gv'
...
for connection_dic in connections_list_of_dics:
    ...
    if (connection_dic['from type'] == 'expression'):  # expression -> infrule
        graphviz_file.write(
            connection_dic['from perm index'] +
            " -> " +
            connection_dic['to temp index'] +
            ";\n")
...
physgraf.convert_graphviz_to_pictures_with_and_without_labels(
    graphviz_filename, extension, output_path + which_derivation_to_make_no_spaces)
