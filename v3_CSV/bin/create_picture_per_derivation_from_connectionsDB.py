...
for feed in set_of_feeds:
    graphviz_file.write(
        feed + " [shape=ellipse,color=red,image=\"" + feed_pictures + "/" + feed + "." + extension + "\",labelloc=b,URL=\"http://feed.com\"];\n")

set_of_expr = physgraf.set_of_expr_from_list_of_dics(connections_list_of_dics)
for expr in set_of_expr:
    graphviz_file.write(
        expr + " [shape=ellipse,color=red,image=\"" + expr_pictures + "/" + expr + "." + extension + "\",labelloc=b,URL=\"http://expre.com\"];\n")
...
for infrule in set_of_infrule:
    ...
    #   print(infrule_name)
    graphviz_file.write(
        infrule_temp + " [shape=invtrapezium,color=red,image=\"" + infrule_pictures + "/" + infrule_name + "." + extension + "\",labelloc=b,URL=\"http://infrule.com\"];\n")
...
physgraf.convert_graphviz_to_pictures_with_and_without_labels(graphviz_filename, \
                                                              extension,
                                                              output_path + which_derivation_to_make_no_spaces)
