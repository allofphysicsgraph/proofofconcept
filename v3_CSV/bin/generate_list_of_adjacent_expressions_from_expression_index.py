...
connections_list_of_dics = physgraf.convert_connections_csv_to_list_of_dics(
    connectionsDB)
expr_list_of_dics = physgraf.convert_expressions_csv_to_list_of_dics(
    expressionsDB)

target_latex = convert_expr_perm_indx_to_latex(
    expr_list_of_dics, target_expression)
if (target_latex is None):
...
# the "temp index" for both declareInitialExpression and
# declareFinalExpression only appear once

print(" ")
for this_infrule in list_of_infrules:
    for this_connection in connections_list_of_dics:
        if (this_connection["from temp index"] == this_infrule):
            if (this_connection["from perm index"]
                    == 'declareInitialExpression'):
                continue  # skip init eq
            adjacent_latex = convert_expr_perm_indx_to_latex(
                expr_list_of_dics, this_connection["to perm index"])
            print(
                "                   from " +
                this_connection["from perm index"] +
                " to " +
                adjacent_latex +
                "\n")
...
#       print("from "+this_connection["from perm index"])
            adjacent_latex = convert_expr_perm_indx_to_latex(
                expr_list_of_dics, this_connection["from perm index"])
            print(
                "from " +
                adjacent_latex +
                " to " +
                this_connection["to perm index"])
# end of file
