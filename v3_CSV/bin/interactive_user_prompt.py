...
def start_new_derivation(
    ...
    derivation_name = get_text_input(
        'name of new derivation (can contain spaces): ')
    ...
    while(not done_with_steps):
        ...
        [selected_infrule_dic,
         input_ary,
         feed_ary,
         output_ary] = get_step_arguments(list_of_infrules,
                                          infrule_list_of_dics,
                                          list_of_expr,
                                          connection_expr_temp,
                                          list_of_feeds,
                                          step_ary)
        step_dic = {
            "infrule": selected_infrule_dic["inference rule"],
            "input": input_ary,
            "feed": feed_ary,
            "output": output_ary}
        print("\nResulting dic:")
...
def write_steps_to_file(derivation_name, step_ary, connection_expr_temp,
    ...
    for step_indx, this_step in enumerate(step_ary):
        step_ary[step_indx]['infrule temp indx'] = physgraf.find_new_indx(
            connection_infrule_temp, 1000000, 9999999, "inf rule temp indx")

        for input_indx, input_dic in enumerate(step_ary[step_indx]['input']):
            step_ary[step_indx]['input'][input_indx]['temp indx'] = physgraf.find_new_indx(
                connection_expr_temp, 1000000, 9999999, "expression temporary index: ")
        for output_indx, output_dic in enumerate(
                step_ary[step_indx]['output']):
            step_ary[step_indx]['output'][output_indx]['temp indx'] = physgraf.find_new_indx(
                connection_expr_temp, 1000000, 9999999, "expression temporary index: ")
        for feed_indx, feed_dic in enumerate(step_ary[step_indx]['feed']):
            step_ary[step_indx]['feed'][feed_indx]['feed indx'] = physgraf.find_new_indx(
                list_of_feeds, 1000000, 9999999, "feed temporary index: ")
    ...
    for step_indx, this_step in enumerate(step_ary):
        for input_indx, this_input in enumerate(this_step['input']):
            f.write("\"" +
                    ...
                    str(step_indx) +
                    ",\"expression\"," +
                    str(this_input['temp indx']) +
                    "," +
                    str(this_step['input'][0]['indx']) +
                    ",\"infrule\"," +
                    str(this_step['infrule temp indx']) +
                    "," +
                    this_step['infrule'] +
                    "\n")
        ...
        for output_indx, this_output in enumerate(this_step['output']):
            f.write("\"" +
                    ...
                    this_step['infrule'] +
                    ",\"expression\"," +
                    str(this_output['temp indx']) +
                    "," +
                    str(this_step['output'][0]['indx']) +
                    "\n")
    f.close()
...
def select_from_available_derivations(list_of_derivations):
    ...
    while(not choice_selected):
        ...
        if (derivation_choice_input == '0' or derivation_choice_input == ''):
            ...
            try:
                selected_derivation = list_of_derivations[
                    int(derivation_choice_input) - 1]
                ...
                print("--> invalid choice (should be in range 0," +
                      str(len(list_of_derivations)) + "); try again")
                time.sleep(3)
...
#       middle_menu=" "*(50-len(list_of_infrules[indx-1]))+str(middle_indx)+"   "+list_of_infrules[middle_indx-1]
            middle_menu = " " * \
                (50 - len(list_of_infrules[indx - 1])) + "   " + list_of_infrules[middle_indx - 1]
            ...
            if (right_side_indx < (len(list_of_infrules) + 1)):
                ...
                right_side_menu = " " * \
                    (40 - len(list_of_infrules[middle_indx - 1])) + "   " + list_of_infrules[middle_indx - 1]
        ...
        # readline.parse_and_bind('tab: complete') # works on Linux, not Mac OS
        # X
    ...
    while(not valid_input):
        ...
        if (int(latex_or_index_choice) == 1):
            this_latex = get_text_input(type_str +
                                        ...
                                        ': ')
            expr_perm_indx = physgraf.find_new_indx(
                list_of_expr, 1000000000, 9999999999, "expression permanent index: ")
            valid_input = True
        elif (int(latex_or_index_choice) == 2):
            expr_perm_indx = get_numeric_input(
                "expression index : ", "defaulllt")
            this_latex = "NONE"
            ...
            if (this_latex == "NONE"):
                print(
                    "ERROR: user-supplied expression index not found in this derivation")
            valid_input = True
        else:
...
def user_provide_latex_arguments(
        selected_infrule_dic,
        step_ary,
        connection_expr_temp):
    ...
    if (number_of_input_expressions > 0):
        for input_indx in range(number_of_input_expressions):
            this_input_dic = user_supplies_latex_or_expression_index(
                'input', input_indx, number_of_input_expressions, list_of_expr, step_ary)
    ...
    if (number_of_output_expressions > 0):
        for output_indx in range(number_of_output_expressions):
            this_output_dic = user_supplies_latex_or_expression_index(
                'output', output_indx, number_of_output_expressions, list_of_expr, step_ary)
...
def get_step_arguments(list_of_infrules, infrule_list_of_dics, list_of_expr,
    ...
    selected_infrule_dic = user_choose_infrule(
        list_of_infrules, infrule_list_of_dics)
    clear_screen()
    [input_ary, feed_ary, output_ary] = user_provide_latex_arguments(
        selected_infrule_dic, step_ary, connection_expr_temp)
