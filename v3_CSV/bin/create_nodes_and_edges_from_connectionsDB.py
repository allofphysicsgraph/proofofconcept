...
print("number of unique nodes in the graph = " +
      str(len(set(connection_feeds + connection_expr_temp + connection_infrule_temp))))
...
for this_deriv in list_of_deriv:
    #   print(this_deriv)
    this_connections_list_of_dics = physgraf.keep_only_this_derivation(
...
# this contains a bunch of redundant entries. However, neither dics nor
# lists are hashable to determine collisions
print('number of nodes found: ' + str(len(list_of_nodes)))
...
for this_node_dic in pruned_list_of_nodes:
    nodes_file.write(
        this_node_dic['temp index'] +
        "," +
        this_node_dic['type'] +
        "," +
        this_node_dic['perm index'] +
        "\n")
...
for this_edge_dic in list_of_edges:
    edges_file.write(
        this_edge_dic['source'] +
        "," +
        this_edge_dic['destination'] +
        "\n")
edges_file.close()
