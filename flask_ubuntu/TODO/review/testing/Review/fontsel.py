...
from encoding import EncodingDB
...
def create_DVI_font(fontname, k, s, d, findenc):
        ...
        if method == 'c':  # load from cache
            ...
            if enc_list:
                if len(enc_list) == 1:  # ok(?), one enc file
                    ...
                    raise FontError(
                        "Guess encoding: found more then one encoding file that matches '%s'.  Here is a list of files: %s" %
                        (fontname, tmp))
    ...
    if not enc:
        raise FontError(
            "Can't determine encoding of font '%s'.  Provide this information with --enc switch." %
            fontname)

    if 'c' not in findenc and new:
        log.debug("Adding information about new font to '%s': %s@%fpt, enc. %s" % (
            setup.font_lookup, font.name, font.designsize, enc))
...
def get_char(fontnum, dvicode):
    ...
    try:
        ...
        log.error("%s: missing char '%s' (%d=0x%02x)",
                  font.name, glyphname, dvicode, dvicode)
        raise e
...
def preload(enc_repl={}):
    ...
    mftrace_available = findfile.which('mftrace') is not None
    fontforge_available = findfile.which('fontforge') is not None
    fnt2meta_available = findfile.which('fnt2meta') is not None
    ...
    for fontname, newenc in enc_repl.iteritems():
        if fontname not in config.fonts_lookup:
            ...
            if enc != newenc:
                log.debug(
                    "Encoding of '%s' chenged to %s (from %s)",
                    fontname,
                    newenc,
                    enc)
...
def load_font(fontname):
    try:
        ...
        if not os.path.exists(filename):
            log.debug(
                "Cache file for '%s' font does not exists -- creating one" %
                fontname)
    ...
    for i, line in enumerate(file):
        ...
        try:
            ...
            if fontname not in config.fonts_lookup:
                ...
                log.warning(
                    "... line %d: font %s - duplicated entry ignored" %
                    (i + 1, fontname))
        except ValueError:
            # wrong number of fields
            log.waring(
                "... line %d has wrong format, skipping" %
                (file.name, i + 1))
            continue
...
def get_encoding_from_MAPfiles(fontname):
    ...
    if tex_map_file_list is not None:
...
def guess_encoding(font_names):
    ...
    if enc_file_list is not None:
        ...
        for file_path in enc_files:
            ...
            except encoding.ENCFileError as e:
...
def make_cache_file(fontname):
    ...
    try:
        ...
        if fontface.getAttribute(
                'bbox') and fontface.getAttribute('units-per-em'):
            xmin, ymin, xmax, ymax = map(
                float, fontface.getAttribute('bbox').replace(
                    ",", " ").split())
...
#			raise str((upem, (ymax-ymin)))
    ...
    for node in fontnode.getElementsByTagName('glyph'):
        ...
        elif default_hadvx is not None:
            glyph.hadv = default_hadvx
        else:
            # XXX: calculate glyphs extends?
            raise FontError(
                "Can't determine width of character '%s'",
                glyph.name)
...
def mftrace_convert(mffile):
    ...
    # just name is needed, no path, no extension
    mfname = os.path.split(mffile)[1][:-3]
