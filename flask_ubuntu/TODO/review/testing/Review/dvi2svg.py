...
class SVGTextDocument(SVGGfxDocument):
    ...
    def flush_chars(self):
        ...
        for (fntnum, char_list) in byfntnum:
            ...
            style = "font-family:%s; font-size:%0.1fpt" % (
                fnt.fontfamily, fnt.designsize)
            s, d = fnt.sd
            if s != d:  # scaled font
            ...
            def isglyphknown(fntnum, dvicode):
                try:
                    return bool(
                        name_lookup[font.get_char_name(fntnum, dvicode)])
                except KeyError:
                    return False
            ...
            def output_char_string(list):
                ...
                color = list[0][5]
                text = ''.join(
                    [name_lookup[font.get_char_name(item[0], item[1])] for item in list])
            ...
            for (
                ...
                if not known:
                    ...
                    for (
                        ...
                        if set_char == 'set':
                            for (
                                    color, char_list4) in group(
                                    char_list3, lambda x: x[5]):
...
def convert_page(dvi, document):
    ...
    while dvi:
        ...
        if command == 'set_char':
            ...
            # fntnum = fntnum_recode[arg] # recode fntnum, see comment marked
            # with (A)
            fntnum = arg
            ...
            raise ValueError(
                "'pre' command is not allowed inside page - DVI corrupted")
        elif command == "post":
            raise ValueError(
                "'post' command is not allowed inside page - DVI corrupted")
...
if __name__ == '__main__':
    ...
    for filename, basename in args:
        ...
        if mag == 1000:  # not scaled
            ...
            log.debug(
                "%s ('%s') has %d page(s); magnification is %0.2f",
                dvi.name,
                comment,
                len(page_offset),
                mag / 1000.0)
        ...
        for k in fonts:
            ...
            try:
                ...
                font.create_DVI_font(
                    fontname, k, s, d, setup.options.enc_methods)
            except font.FontError as e:
                log.error("Can't find font '%s': %s" % (fontname, str(e)))
                missing.append((k, fontname))

        if missing:
            log.info(
                "There were some unavailable fonts, skipping file '%s'; list of missing fonts: %s" %
                (dvi.name, ", ".join(
                    "%d=%s" %
                    kf for kf in missing)))
            continue
        ...
        if setup.options.pages is None:  # processing all pages
            ...
            try:
                ...
                log.warning(
                    "Argument of --pages switch has invalid syntax ('%s'), processing first page",
                    setup.options.pages)
                pages = [0]
        ...
        if setup.options.single_file:
            svg = SVGDocument(
                ...
                scale,
                unit_mm,
                setup.options.paper_size)
            for i, pageno in enumerate(pages):
                log.info(
                    "Procesing page %d (%d of %d)",
                    pageno + 1,
                    i + 1,
                    len(pages))
            ...
            for i, pageno in enumerate(pages):
                ...
                dvi.seek(page_offset[pageno])
                svg = SVGDocument(1.25 * mag, scale, unit_mm,
                                  setup.options.paper_size)
                svg.new_page()
                convert_page(dvi, svg)
