...
json_str_edges_and_nodes = line_of_edges_and_nodes[24:len(
    line_of_edges_and_nodes) - 4]
...
for line_indx, this_line in enumerate(trimmed_list_of_lines):
    if (this_line == "\\begin{equation}"):
        expr_dic = {}
        search_object = re.search(
            r'[0-9]{4}', trimmed_list_of_lines[line_indx + 2])
        node_ID = search_object.group()

        this_line_is_expr = True
        for list_indx, this_node in enumerate(list_of_nodes):
            if (node_ID == this_node['id'] and this_node['type']
                    == 'text'):  # node is infrule, not expression
