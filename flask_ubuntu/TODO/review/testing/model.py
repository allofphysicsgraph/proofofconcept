...
class AddAnotherStep(Form):
    addStep = SelectField(
        u'Add another step?', choices=[
            ('y', 'yes'), ('n', 'no, produce file')])


class SelectInfRule(
        Form):  # http://wtforms.simplecodes.com/docs/0.6.1/fields.html
    inference_rule = SelectField(
        u'inference rule:',
        choices=[
            ('multbothsidesby',
