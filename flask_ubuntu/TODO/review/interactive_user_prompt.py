...
# for determining image dimensions, necessary for JSON which is referenced
# by d3js webpage
...
def create_pictures_for_derivation(output_path, derivation_name):
    ...
    for name in os.listdir(output_path + '/' + derivation_name + '/'):
        ...
        if (os.path.isfile(output_path + '/' + derivation_name +
                           '/' + name) and (name_and_extension[1] == 'tex')):
            ...
            with open(output_path + '/' + derivation_name + '/' + name, 'r') as fderiv:
                ...
                make_picture_from_latex_expression(
                    numeric_as_ary[0],
                    output_path +
                    '/' +
                    derivation_name,
...
@track_function_usage
def make_picture_from_latex_expression(
        file_name,
        folder_name,
        latex_expression,
        extension):
    ...
    if (os.path.isfile(tmp_tex)):
        os.remove(tmp_tex)
    # here's why the "h" needs a double escape:
    # https://stackoverflow.com/questions/18174827/what-does-h-mean-in-a-python-format-string
    tex_string = "\n\\thispagestyle{empty}\n\\begin{document}\n\\huge{" + \
                 latex_expression + "}\n\\end{document}\n"
    # first argument is a text size, the further arguments set the
    # corresponding math sizes in display/text style, script style and
    # scriptscript style.
    ...
    if (extension == 'svg'):
        os.system(
            "python lib/pydvi2svg/dvi2svg.py --paper-size=bbox:10 tmp.dvi &> /dev/null")
        os.system("mv tmp.svg \"" + path_to_file + "\"")
    # os.system("convert "+path_to_file+" "+path_to_file) # for some reason
    # the initial svg format isn't interperatable by graphviz. Thus, I pass
    # the .svg through convert
...
@track_function_usage
def write_edges_and_nodes_to_graphviz(
    ...
    for this_pair in infrule_list:
        if (len(this_pair) != 2):
            ...
            if (this_pair[1] ==
                    "declareInitialExpr" and not found_declare_init):
                found_declare_init = True
                # graphviz_file.write(this_pair[0]+" [shape=invtrapezium, color=red,
                # label=\""+this_pair[1]+"\", pos=\"0,0\"];\n")    #
...
def create_graph_for_derivation(output_path, derivation_name):
    ...
    edge_list, expr_list, infrule_list, feed_list = read_csv_files_into_ary(
        which_derivation_to_make)
    write_edges_and_nodes_to_graphviz(
        which_derivation_to_make,
        edge_list,
        expr_list,
        infrule_list,
...
def first_choice(
    ...
    while (invalid_choice):
        ...
        if (first_choice_input == '0' or first_choice_input == ''):
            ...
            derivation_name = start_new_derivation(
                list_of_infrules_fc, infrule_list_of_dics, output_path)
            invalid_choice = False
    ...
    while (invalid_choice):
        ...
        if (first_choice_input == '0'):
            ...
            for selected_derivation in list_of_derivations:
                generate_webpage_for_single_derivation(
                    selected_derivation, output_path)
            invalid_choice = False
        elif (first_choice_input == '2'):
            # TODO
            ...
            [derivation_choice_input, selected_derivation] = select_from_available_derivations(
                list_of_derivations)
            generate_webpage_for_single_derivation(
                selected_derivation, output_path)
            invalid_choice = False
...
def create_json_for_webpage(path_to_pictures, path_to_json, step_ary):
    ...
    img_width, img_height = image_dimensions(
        path_to_pictures + "/" + feed + ".png")
    node_lines += "  {\"img\": \"" + path_to_pictures + "/" + feed + ".png\", \"width\": " + \
                  img_width + ", \"height\": " + img_height + ", \"label\": \"" + feed + "\"},\n"
...
def generate_webpage_for_single_derivation(selected_derivation, output_path):
    ...
    step_ary = read_derivation_steps_from_files(
        selected_derivation, output_path)
...
def generate_PDF(list_of_derivations, infrule_list_of_dics, output_path):
    ...
    while (invalid_choice):
        ...
        if (first_choice_input == '0'):
            ...
            for selected_derivation in list_of_derivations:
                generate_pdf_for_single_derivation(
                    selected_derivation, output_path)
            invalid_choice = False
        elif (first_choice_input == '2'):
            print("currently there is no action associated with this selection")
            ...
            [derivation_choice_input, selected_derivation] = select_from_available_derivations(
                list_of_derivations)
            generate_pdf_for_single_derivation(
                selected_derivation, infrule_list_of_dics, output_path)
            invalid_choice = False
        else:
            print(first_choice_input)
            print("\n--> invalid choice; try again")
...
@track_function_usage
def generate_pdf_for_single_derivation(
        selected_derivation,
        infrule_list_of_dics_gps,
        output_path):
    '''
    see
    proofofconcept/v3_CSV/bin/create_pdf_per_derivation_from_connectionsDB.py
    ...
    for this_step_dict in step_ary:
        ...
        for this_input_dict in this_step_dict['input']:
            strng_for_this_step += str(
                this_input_dict['indx specific to this step for input']) + "}{"
        ...
        for this_output_dict in this_step_dict['output']:
            strng_for_this_step += str(
                this_output_dict['indx specific to this step for input']) + "}{"
        # remove last character which is a {
        strng_for_this_step = strng_for_this_step[:-1]
        strng_for_this_step += "\n"
        ...
        for this_output_dict in this_step_dict['output']:
            ...
            tex_file.write(
                "\\label{eq:" + str(this_output_dict['indx specific to this step for input']) + "}\n")
...
def latex_header(tex_file):
    ...
    tex_file.write(
        '\\usepackage[backref, colorlinks=false, pdftitle={' +
        todays_date +
        '}, \n')
...
def combine_two_derivations(list_of_derivations, output_path):
    clear_screen()
    # pick a derivation
    [first_derivation_choice_input,
     first_selected_derivation] = select_from_available_derivations(list_of_derivations)
    if (first_selected_derivation == 'EXIT'):
        print("exit from editing")
        return
    step_ary_for_first_derivation = read_derivation_steps_from_files(
        first_selected_derivation, output_path)
    ...
    step_ary_for_second_derivation = read_derivation_steps_from_files(
        second_selected_derivation, output_path)
    ...
    for this_step_dic in step_ary_for_first_derivation:
        print("==== step index: " +
              str(this_step_dic['infrule indx']) + " ====")
        list_of_infrule_indices.append(
            str(this_step_dic['infrule indx']))  # str for tab completion
        print_this_step(this_step_dic)

    selected_step_indx = select_step_from_derivation(
        first_selected_derivation, list_of_infrule_indices)
    print("\nselected step:")
...
def popularity_counts(list_of_derivations_pc, path):
    ...
    # cut -d',' -f2  | xargs -I % sh -c 'cat expressions/%_latex_*' | sort |
    # uniq -c | sort -g -k1,1
    ...
    for infrule in sorted(
            infrule_count_dict, key=infrule_count_dict.get):  # , reverse=True
    ...
    for expr_indx in sorted(
            expr_count_dict, key=expr_count_dict.get):  # , reverse=True
        # only print the first line of the latex
        print(str(expr_count_dict[expr_indx]) + " " + expr_dict[expr_indx][0])
    print("\n")
...
def edit_existing_derivation(
        list_of_derivations_eed,
        output_path,
        list_of_infrules_eed,
        infrule_list_of_dics_eed):
    ...
    while (invalid_choice):
        ...
        if (input_choice == "a"):
            print("add new step")
            invalid_choice = False
            step_dic = get_step_arguments(
                list_of_infrules_eed, infrule_list_of_dics_eed, step_ary)
            step_ary.append(step_dic)
        elif (input_choice == "d"):
            print("delete step")
            invalid_choice = False
            step_ary = delete_step_from_derivation(
                step_ary, selected_derivation, list_of_infrule_indices)
        elif (input_choice == "e" or input_choice == ""):
            print("edit step")
            invalid_choice = False
            step_ary = edit_step_in_derivation(
                step_ary,
                selected_derivation,
                list_of_infrule_indices,
                list_of_infrules_eed,
...
@track_function_usage
def edit_step_in_derivation(
        step_ary,
        selected_derivation,
        list_of_infrule_indices,
        list_of_infrules_esd,
        infrule_list_of_dics_esd):
    selected_step_indx = select_step_from_derivation(
        selected_derivation, list_of_infrule_indices)
    print("\nstep to edit:")
    ...
    while (invalid_choice):
        ...
        if (input_choice == "r"):
            ...
            new_infrule = change_inference_rule_in_step(
                step_to_edit, list_of_infrules_esd, infrule_list_of_dics_esd)
            for this_step in step_ary:
                if (this_step['infrule indx'] == int(selected_step_indx)):
        ...
        # step_to_edit.keys() = ['infrule', 'input', 'infrule indx', 'feed',
        # 'output']
            ...
            if (len(step_to_edit['input']) == 0):
                ...
                invalid_choice = False
                edited_step = change_input_output_feed_in_step(
                    step_to_edit, 'input')
        elif (input_choice == "f"):
            print("edit feed")
            if (len(step_to_edit['feed']) == 0):
...
@track_function_usage
def change_inference_rule_in_step(
        step_to_edit,
        list_of_infrules_cirs,
        infrule_list_of_dics_cirs):
    print(step_to_edit)
    print("inf rule is currently " + step_to_edit['infrule'])
    selected_infrule_dic = user_choose_infrule(
        list_of_infrules_cirs,
        infrule_list_of_dics_cirs,
        len_of_step_ary=10)  # len_of_step_ary should be greater than 1 to avoid the default of declareInit
...
def change_input_output_feed_in_step(step_to_edit, which_category):
    ...
    while (not text_provided):
        this_latex = get_text_input(
            "enter new latex for " + which_category + ": ", True)
        text_provided = True
...
def delete_step_from_derivation(
    ...
    for step_indx, this_step in enumerate(step_ary):
        ...
        if (str(step_ary[step_indx]['infrule indx'])
                == str(selected_step_indx)):
...
def start_new_derivation(
        list_of_infrules_snd,
        infrule_list_of_dics_snd,
        output_path):
    clear_screen()
    print("starting new derivation")
    derivation_name = get_text_input(
        'name of new derivation (can contain spaces): ', True)
    ...
    while (not done_with_steps):
        ...
        done_with_steps = add_another_step_menu(
            step_ary, derivation_name, output_path)
...
def check_this_step(this_step_dic):
    ...
    # for the following derivations, each takes an input and a feed and
    # produces an output
    if (len(this_step_dic['input']) > 0):
        # list length is 1 since there is a single input
        input_latex = this_step_dic['input'][0]['latex']
    if (len(this_step_dic['output']) > 0):
        # list length is 1 since there is a single output
        output_latex = this_step_dic['output'][0]['latex']
    if (len(this_step_dic['feed']) > 0):
        # list length is 1 since there is a single feed
        feed_latex = this_step_dic['feed'][0]['latex']
...
def print_this_step(this_step_dic):
    ...
    # inference rule appears after inputs and before outputs
...
def expr_indx_exists_in_ary(test_indx, test_step_indx, step_ary):
    ...
    for step_indx, this_step in enumerate(step_ary):
        ...
        if (test_step_indx != step_indx):
            for input_indx, input_dic in enumerate(
                    step_ary[step_indx]['input']):
            ...
            for output_indx, output_dic in enumerate(
                    step_ary[step_indx]['output']):
...
def assign_temp_indx(step_ary):
    ...
    for step_indx, this_step in enumerate(step_ary):
        if 'infrule indx' not in step_ary[step_indx].keys():
            step_ary[step_indx]['infrule indx'] = get_new_step_indx(
                '../../../DataSource/derivations')
        ...
        for input_indx, input_dic in enumerate(step_ary[step_indx]['input']):
            if ('indx specific to this step for input' not in input_dic.keys()):
                ...
                temp_indx_for_this_expr_indx = expr_indx_exists_in_ary(
                    expr_indx, step_indx, step_ary)
                if (temp_indx_for_this_expr_indx == 0):
                    step_ary[step_indx]['input'][input_indx][
                        'indx specific to this step for input'] = get_new_step_indx(
                        '../../../DataSource/derivations')
                else:
                    step_ary[step_indx]['input'][input_indx][
                        'indx specific to this step for input'] = temp_indx_for_this_expr_indx
        ...
        for output_indx, output_dic in enumerate(
            ...
            if 'indx specific to this step for output' not in output_dic.keys():
                ...
                temp_indx_for_this_expr_indx = expr_indx_exists_in_ary(
                    expr_indx, step_indx, step_ary)
                if (temp_indx_for_this_expr_indx == 0):
                    step_ary[step_indx]['output'][output_indx][
                        'indx specific to this step for output'] = get_new_step_indx(
                        '../../../DataSource/derivations')
                else:
                    step_ary[step_indx]['output'][output_indx][
                        'indx specific to this step for output'] = temp_indx_for_this_expr_indx

        for feed_indx, feed_dic in enumerate(step_ary[step_indx]['feed']):
            step_ary[step_indx]['feed'][feed_indx]['feed indx'] = get_new_step_indx(
                '../../../DataSource/derivations')
...
{'infrule': 'addZerotoLHS',
 ...
 'output': [
    ...
    for indx_for_expr in expr_list:
        ...
        exprfile = None
        if os.path.exists(output_path + "/" + derivation_name +
                          "/" + str(indx_for_expr[1]) + "_latex.tex"):
            exprfile = open(output_path + "/" + derivation_name +
                            "/" + str(indx_for_expr[1]) + "_latex.tex")
        else:
            #            print("reached the else block")
            list_of_tex_files = glob(
                "expressions/" + str(indx_for_expr[1]) + "_latex_*.tex")
            ...
            if (len(list_of_tex_files) == 0):
                ...
                print(
                    "multiple expression Latex files found for " +
                    str(indx_for_expr))
                print(list_of_tex_files)
        ...
        if (exprfile is not None):
            ...
            this_dic['indx specific to this step for input'] = int(
                indx_for_expr[0])
    ...
    for this_step in step_ary:
        for this_edge in edge_list_typed:
            if (this_step['infrule indx'] == this_edge[0]):
                for this_expr in list_of_expr_dics:
                    if (this_expr['indx specific to this step for input']
                            == this_edge[1]):
...
def write_steps_to_file(derivation_name, step_ary, output_path):
    ...
    infrule_file = open(
        ...
        derivation_name +
        '/inference_rule_identifiers.csv',
        'w+')  # infrule_ID and inf_rule
    feed_file = open(
        output_path + '/' + derivation_name + '/feeds.csv',
        'w+')  # feed_ID
    edgelist_file = open(
        output_path +
        '/' +
        derivation_name +
        '/derivation_edge_list.csv',
        'w+')  # step_ID and step_ID
    expr_file = open(
        output_path +
        '/' +
        derivation_name +
        '/expression_identifiers.csv',
        'w+')  # expr_ID and step_ID

    list_of_expr_dics = []
    for step_indx, this_step in enumerate(step_ary):
        ID_for_this_step = get_new_expr_indx('../../../DataSource/derivations')

        infrule_file.write(
            str(ID_for_this_step) +
            "," +
            this_step['infrule'] +
            "\n")
        ...
        for this_feed_dic in this_step['feed']:
            ...
            edgelist_file.write(
                str(this_feed_dic['feed indx']) + ',' + str(ID_for_this_step) + "\n")
            filename = output_path + '/' + derivation_name + '/' + \
                       str(this_feed_dic['feed indx']) + '_latex.tex'
        ...
        for this_input_dic in this_step['input']:
            ...
            # this approach yields duplicate lines in the expression_identifiers.csv
            # because the same expression gets referenced repeatedly in a derivation
            edgelist_file.write(
                str(
                    this_input_dic['indx specific to this step for input']) +
                ',' +
                str(ID_for_this_step) +
                "\n")
        ...
        for this_output_dic in this_step['output']:
            ...
            # this approach yields duplicate lines in the expression_identifiers.csv
            # because the same expression gets referenced repeatedly in a derivation
            edgelist_file.write(str(ID_for_this_step) + ',' + str(
                this_output_dic['indx specific to this step for output']) + "\n")

    for this_expr_dic in list_of_expr_dics:
        if "indx specific to this step for output" in this_expr_dic.keys():
            expr_file.write(str(this_expr_dic['indx specific to this step for output']) + "," + str(
                this_expr_dic['expression indx']) + "\n")
        if "indx specific to this step for input" in this_expr_dic.keys():
            expr_file.write(str(this_expr_dic['indx specific to this step for input']) + "," + str(
                this_expr_dic['expression indx']) + "\n")
        filename = output_path + '/' + derivation_name + '/' + \
                   str(this_expr_dic['expression indx']) + '_latex.tex'
...
def select_from_available_derivations(list_of_derivations_sad):
    ...
    while (not choice_selected):
        ...
        write_activity_log(
            "user selected " +
            str(derivation_choice_input),
            "select_from_available_derivations")
        if (derivation_choice_input == '0' or derivation_choice_input == ''):
            ...
            try:
                selected_derivation = list_of_derivations_sad[int(
                    derivation_choice_input) - 1]
            ...
            #        time.sleep(3)
                ...
                print("WARNING: invalid choice (should be in range 0," +
                      str(len(list_of_derivations_sad)) + "); try again")
    return int(derivation_choice_input), selected_derivation


@track_function_usage
def user_choose_infrule(
        list_of_infrules_uci,
        infrule_list_of_dics_uci,
        len_of_step_ary):
    ...
    while (not choice_selected):
        clear_screen()
        print("choose from the list of " +
              str(len(list_of_infrules_uci)) +
              " inference rules")
        num_left_col_entries = int(
            ceil(
                len(list_of_infrules_uci) /
                3.0) +
            1)  # number of rows
        num_remaining_entries = len(
            list_of_infrules_uci) - num_left_col_entries
        list_of_infrules_uci = sorted(
            list_of_infrules_uci,
            key=lambda s: s.lower())  # this is case-insensitive
        # list_of_infrules_uci.sort() # this is case-sensitive; the .sort() method
        # places capitalized before lowercase
        for indx in range(1, num_left_col_entries):
            ...
            middle_menu = " " * (number_of_spaces - len(
                list_of_infrules_uci[indx - 1])) + "   " + list_of_infrules_uci[middle_indx - 1]
            ...
            if (right_side_indx < (len(list_of_infrules_uci) + 1)):
                ...
                right_side_menu = " " * (number_of_spaces - len(
                    list_of_infrules_uci[middle_indx - 1])) + "   " + list_of_infrules_uci[right_side_indx - 1]
        ...
        # see also https://pypi.python.org/pypi/gnureadline, though I didn't
...
def get_list_of_expr_indices_and_latex(step_ary):
    ...
    for this_step in step_ary:
        for this_input_dic in this_step['input']:
            dic_of_expr_and_latex[str(
                this_input_dic['expression indx'])] = this_input_dic['latex']
        for this_output_dic in this_step['output']:
            dic_of_expr_and_latex[str(
                this_output_dic['expression indx'])] = this_output_dic['latex']
    return dic_of_expr_and_latex


@track_function_usage
def user_supplies_latex_or_expression_index(
        type_str, input_indx, number_of_expressions, step_ary):
    ...
    while (not valid_input):
        ...
        if (int(latex_or_index_choice) == 1):
            text_provided = False
            while (not text_provided):
                this_latex = get_text_input(type_str +
                                            ' expression Latex,    ' +
                                            str(input_indx +
                                                1) +
                                            ' of ' +
                                            str(number_of_expressions) +
                                            ': ', True)
                if (("=" not in this_latex) and (">" not in this_latex)
                        and ("<" not in this_latex)):
                    text_provided = False
                    print(
                        "--> invalid input (missing relation operator); Enter a string")
                else:
                    text_provided = True

            expr_comment = get_text_input("comment: ", False)
            expr_ID = get_new_expr_indx('../../../DataSource/derivations')

            valid_input = True
        elif (int(latex_or_index_choice) == 2):
            dic_of_expr_and_latex = get_list_of_expr_indices_and_latex(
                step_ary)
            ...
            if (this_latex == "NONE"):
                print(
                    "ERROR [in interactive_user_prompt.py, user_supplies_latex_or_expression_index: user-supplied expression index not found in this derivation")
                valid_input = False
            valid_input = True
        else:
...
def user_provide_latex_arguments(selected_infrule_dic, step_ary):
    ...
    number_of_input_expressions = int(
        selected_infrule_dic['number of input expressions'])
    ...
    number_of_output_expressions = int(
        selected_infrule_dic['number of output expressions'])

    print(
        "number of input expressions: " +
        str(number_of_input_expressions) +
        ", number of feeds: " +
        str(number_of_feeds) +
        ", number of output expressions: " +
        str(number_of_output_expressions))
    ...
    if (number_of_input_expressions > 0):
        for input_indx in range(number_of_input_expressions):
            this_input_dic = user_supplies_latex_or_expression_index(
                'input', input_indx, number_of_input_expressions, step_ary)
    ...
    if (number_of_feeds > 0):
        for feed_indx in range(number_of_feeds):
            ...
            feed_dic['latex'] = get_text_input(
                'feed Latex,                ' + str(
                    feed_indx + 1) + ' of ' + str(number_of_feeds) + ': ', True)
            feed_ary.append(feed_dic)
    output_ary = []
    if (number_of_output_expressions > 0):
        for output_indx in range(number_of_output_expressions):
            this_output_dic = user_supplies_latex_or_expression_index(
                'output', output_indx, number_of_output_expressions, step_ary)
...
@track_function_usage
def get_step_arguments(
        list_of_infrules_gsa,
        infrule_list_of_dics_gsa,
        step_ary):
    print("starting a new step")
    selected_infrule_dic = user_choose_infrule(
        list_of_infrules_gsa, infrule_list_of_dics_gsa, len(step_ary))
    clear_screen()
    [input_ary, feed_ary, output_ary] = user_provide_latex_arguments(
        selected_infrule_dic, step_ary)
    ...
    if (not step_is_valid):
        print(
            "would you like to re-enter this step? [actually going to continue for now.]")
...
def find_input_files(path_for_derivations_fif, path_for_infrules_fif):
    ...
    for this_infrule in list_of_infrules_fif:
        ...
        if not os.path.isfile(
                path_for_infrules_fif +
                '/' +
                this_infrule +
                '_parameters.yaml'):
            print(
                'missing inf rule yaml file for ' +
                this_infrule +
                '_parameters.yaml')
            exit()
        try:
            ...
            print
            "ERROR [in interactive_user_prompt.py, main]: YAML configuration file:", exc

        if (config['inf_rule_name'] != this_infrule):
            print("name of .tex file doesn't match what's in the .yaml file")
            print(
                "the infrule in question is " +
                this_infrule +
                " and the inf_rule_name is")
        ...
        if (len(list_of_tex_files) > 0):
            ...
            this_dic['LaTeX expansion'] = "no tex file found for " + \
                                          this_infrule
...
if __name__ == "__main__":
    ...
    path_for_derivations = '../../../DataSource/derivations'
    path_for_infrules = '../../../DataSource/inference_rules'
    [infrule_list_of_dics, list_of_derivations, list_of_infrules] = find_input_files(
        path_for_derivations, path_for_infrules)
    while (True):
