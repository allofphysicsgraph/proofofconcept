...
def list_connections():
    connection_indx = 1
    for these_connections in connectionsDB.getElementsByTagName(
            'connection_set'):
        connection_name = physgraf.remove_tags(
            these_connections.toxml(), 'connection_set')
...
def list_inference_rules():
    ...
    for these_rules in inference_rulesDB.getElementsByTagName('infrule_name'):
        inf_rule_name = physgraf.remove_tags(
            these_rules.toxml(), 'infrule_name')
