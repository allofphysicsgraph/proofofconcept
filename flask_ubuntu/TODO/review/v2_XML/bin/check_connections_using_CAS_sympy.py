...
def convert_xml_to_cas_array(
        type_str,
        xml_ary,
        symbols_array,
        check_this_connection):
    lhs_array = []
    rhs_array = []
    for stat in xml_ary[0].getElementsByTagName(
            'expression_permenant_unique_id'):
        expression_permenant_unique_id = physgraf.remove_tags(
            stat.toxml(encoding="ascii"), 'expression_permenant_unique_id')
        # now that we know the expression index, retrieve the CAS expressions
        # from expressionsDB

        [expression_lhs, expression_rhs] = physgraf.convert_expression_permenant_unique_id_to_cas_sympy(
            expression_permenant_unique_id, expressionsDB)
        symbol_permenant_unique_id_array = physgraf.convert_tpunid_to_symbol_permenant_unique_id_array(
            expression_permenant_unique_id, expressionsDB, 'expression')
        for indx in range(len(symbol_permenant_unique_id_array)):
            cas_sympy = physgraf.convert_symbol_permenant_unique_id_to_cas_sympy(
                symbol_permenant_unique_id_array[indx], symbolsDB)
...
def write_sympy_to_file(
    ...
    CAS_file.write(
        "# this file was written by check_connections_using_CAS_sympy.py\n")
...
def convert_feed_to_cas_array(
        inputs_xml,
        symbols_array,
        check_this_connection):
    feed_array = []
    for feeed in inputs_xml[0].getElementsByTagName('feed_tunid'):
        feed_tunid = physgraf.remove_tags(
            feeed.toxml(encoding="ascii"), 'feed_tunid')
        feed_sympy = physgraf.convert_feed_tunid_to_feed_sympy(
            feed_tunid, feedsDB)
...
for this_connection in connectionsDB.getElementsByTagName('connection'):
    ...
    this_inference_rule = physgraf.convert_tag_to_content(
        this_connection, 'infrule_name', 0)
    ...
    for inf_rule in inference_rulesDB.getElementsByTagName('inference_rule'):
        ...
        if (inf_rule_name == this_inference_rule):
            inf_rule_sympy = physgraf.convert_tag_to_content(
                inf_rule, 'cas_sympy', 0)
            print(inf_rule_sympy)

    symbols_array = []

    this_infrule_tunid = physgraf.convert_tag_to_content(
        this_connection, 'infrule_tunid', 0)
    ...
    if len(outputs_xml) > 0:
        ...
        [check_this_connection,
         out_lhs_array,
         out_rhs_array,
         symbols_array] = convert_xml_to_cas_array("outputs",
                                                   outputs_xml,
                                                   symbols_array,
                                                   check_this_connection)
    else:
    ...
    if len(inputs_xml) > 0:
        [check_this_connection,
         in_lhs_array,
         in_rhs_array,
         symbols_array] = convert_xml_to_cas_array("inputs",
                                                   inputs_xml,
                                                   symbols_array,
                                                   check_this_connection)

        [check_this_connection, feed_array, symbols_array] = convert_feed_to_cas_array(
            inputs_xml, symbols_array, check_this_connection)
    else:
        in_lhs_array = []
        in_rhs_array = []
    ...
    if (check_this_connection):
        write_sympy_to_file(
            ...
            inf_rule_sympy,
            this_inference_rule)
        # at this point a valid function exists in a python file; need to run
        # it and get result
...
#       exit()
            ...
            print(
                "ERROR: you shouldn't get here! Problem in check_connections_using_CAS_sympy")
            exit()
