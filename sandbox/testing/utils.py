...
def group_elements(seq, value=lambda x: x):
    ...
    """
    def aux(xxx_todo_changeme, curr):
        (vp, L) = xxx_todo_changeme
        vc = value(curr)
        if vc == vp:
            L[-1][1].append(curr)
        else:
...
def parse_enc_methods(list):
    ...
    for method in list.split(","):
        method = method.lower()
        if method in ['cache', 'c']:
            method = 'c'
        elif method in ['tfm', 't']:
            method = 't'
        elif method in ['afm', 'a']:
            method = 'a'
        elif method in ['map', 'm']:
            method = 'm'
        elif method in ['guess', 'g', 'e']:
            method = 'g'
        else:
