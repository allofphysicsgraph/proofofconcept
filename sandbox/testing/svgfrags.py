...
def main(args):
        ...
        for item in parse(input):
            ...
            if kind == 'string':
                ...
                try:
                    ...
                    log.warning(
                        "String '%s' doesn't found in SVG, skipping repl", value)
                ...
                if object:
                    ...
                    log.warning(
                        "Object with id=%s doesn't found in SVG, skipping repl", value)

            else:  # point, rect -- no additional tests needed
                repl_defs[tex] = ((kind, value), tex, options)

    except frags.parse_subst.SyntaxError as e:
    ...
    if not os.path.exists(tmp_filename + ".dvi"):
        ...
        tmp_lines.append(r"\end{document}")
        ...
        if which('latex'):
            ...
            if exitstatus:
                log.error(
                    "LaTeX failed - error code %d; check log file '%s.log'",
                    exitstatus,
                    tmp_filename)
                sys.exit(2)
        else:
        ...
        log.info(
            "File %s not changed, used existing DVI file (%s)",
            input_txt,
            tmp_filename)
    ...
    for k in fonts:
        ...
        try:
            fontsel.create_DVI_font(
                fontname, k, s, d, setup.options.enc_methods)
        except fontsel.FontError as e:
        ...
        for ((kind, value), tex, options) in items:
            ...
            if kind == 'string':
                ...
                if options.scale == 'fit':
                    log.warning(
                        "%s is a text object, can't fit to rectangle", value)
                    sx = sy = 1.0
                else:
                    sx, sy = options.scale
...
if __name__ == '__main__':
    ...
    try:
        main(sys.argv)
    except SystemExit as xxx_todo_changeme:
        (code) = xxx_todo_changeme
        sys.exit(code)
        ...
        if setup.options.print_traceback:
            ...
            print >> sys.stderr, "Unexpeced error - %s: %s" % (
                exception.__name__, str(instance))
