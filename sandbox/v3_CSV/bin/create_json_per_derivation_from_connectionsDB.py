...
expression_pictures_path = input_data[
    "expr_latex_to_pictures_path"] + extension
feed_pictures_path = input_data["feed_latex_to_pictures_path"] + extension
infrule_pictures_path = input_data[
    "infrule_latex_to_pictures_path"] + extension
...
infrule_pictures_json_path = input_data[
    "infrule_pictures_json_path"] + extension
...
def get_image_size(path, filename):
    output = subprocess.check_output(
        "file " + path + "/" + filename + ".png", shell=True)
...
node_lines = ""
set_of_feeds = physgraf.set_of_feeds_from_list_of_dics(
...
for feed in set_of_feeds:
    ...
    node_lines += "  {\"img\": \"" + feed_pictures_json_path + "/" + feed + ".png\", \"width\": " + img_size_dic[
        "img width"] + ", \"height\": " + img_size_dic["img height"] + ", \"label\": \"" + feed + "\"},\n"
...
for expr in set_of_expr:
    ...
    node_lines += "  {\"img\": \"" + expr_pictures_json_path + "/" + expr + ".png\", \"width\": " + img_size_dic[
        "img width"] + ", \"height\": " + img_size_dic["img height"] + ", \"label\": \"" + expr + "\"},\n"
    local_translation_dic[expr] = node_indx
    node_indx += 1

set_of_infrule = physgraf.set_of_infrule_from_list_of_dics(
    connections_list_of_dics)
for infrule in set_of_infrule:
    ...
    node_lines += "  {\"img\": \"" + infrule_pictures_json_path + "/" + infrule_name + ".png\", \"width\": " + \
        img_size_dic["img width"] + ", \"height\": " + img_size_dic["img height"] + ", \"label\": \"" + infrule_temp + "\"},\n"
...
for connection_dic in connections_list_of_dics:
    ...
    if (connection_dic['from type'] == 'feed'):  # feed -> infrule
        edge_lines += "  {\"source\":" + str(
            local_translation_dic[
                connection_dic['from temp index']]) + ",\"target\":" + str(
            local_translation_dic[
                connection_dic['to temp index']]) + "},\n"
    if (connection_dic['from type'] == 'infrule'):  # infrule -> expression
        edge_lines += "  {\"source\":" + str(
            local_translation_dic[
                connection_dic['from temp index']]) + ",\"target\":" + str(
            local_translation_dic[
                connection_dic['to perm index']]) + "},\n"
    if (connection_dic['from type'] == 'expression'):  # expression -> infrule
        edge_lines += "  {\"source\":" + str(
            local_translation_dic[
                connection_dic['from perm index']]) + ",\"target\":" + str(
            local_translation_dic[
                connection_dic['to temp index']]) + "},\n"
