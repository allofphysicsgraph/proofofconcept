...
{	
	...
	if(!re.test(s)) {alert("Invalid graph code syntax (for example, trailing comma or non-numeric input).\nMust be of the form\n[0-9]+:(|([0-9]+-[0-9]+)(|,[0-9]+-[0-9]+)* "); return;}
	...
	for(var i=0; i<ed.length; i++)
	{
		...
		a = Number(e[0])-1;
		b = Number(e[1])-1;
		if(a >= this.n || b >= this.n) {alert("number of edges inconsistent with number of nodes"); return;}
		this.AddEdge(a, b);
	}
}
