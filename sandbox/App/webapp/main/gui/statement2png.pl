...
while(<EQFILE>) # read each line from input 
{
  ...
  if ( $findComment eq "%" ) { # this line is a comment, so skip it
    ...
    if (($redrawEqPics eq "y") || (($redrawEqPics eq "n") && !(-e $filename))) { # if file does not exist, redraw
      system("perl eq2png '".$latex."' -o ".$filename); # run eq2png to create png version of the latex equation
...
#    print "comment is $comment.\n";
    ...
    if (($redrawOpPics eq "y") || (($redrawOpPics eq "n") && !(-e $filename))) { # if file does not exist, redraw
      system("perl eq2png '".$operator."' -o ".$filename); # run eq2png to create png version of the latex equation
...
#     print "$optionalInputuid\n\n";
    ...
    print MAKEGVFILE "$inputOneuid [shape=ellipse,color=red,image=\"$inputOneuid.png\",labelloc=b,URL=\"http://google.com\"];\n";
    print MAKEGVFILE "$operator [shape=invtrapezium,color=red,image=\"$operator.png\",labelloc=b,URL=\"http://google.com\"];\n";
    print MAKEGVFILE "$outputuid [shape=ellipse,color=red,image=\"$outputuid.png\",labelloc=b,imagescale=true]; \n";
    if ($optionalInputuid != 0) {
      #print "$optionalInputuid\n";
      print MAKEGVFILE "$optionalInputuid [shape=ellipse,color=red,image=\"$optionalInputuid.png\",labelloc=b,imagescale=true]; \n";
