...
def take_curl_of_both_sides(expression, feed, relation):
    ...
    return relation(curl(expression.lhs),
                    curl(expression.rhs), evaluate=False)

def apply_divergence(expression, feed, relation):
    from sympy.vector import divergence
    return relation(divergence(expression.lhs),
                    divergence(expression.rhs), evaluate=False)
...
def indefinite_integration(expression, feed, relation):
    ...
    https://docs.sympy.org/latest/modules/integrals/integrals.html
    """
    return relation(integrate(expression.lhs, feed),
                    integrate(expression.rhs, feed), evaluate=False)
...
def integrate_over_from_to(expression, feed_wrt, feed_upper, feed_lower, relation):
    ...
    https://docs.sympy.org/latest/modules/integrals/integrals.html
    """
    return relation(integrate(expression.lhs, (feed_wrt, feed_lower, feed_upper)),
                    integrate(expression.rhs, (feed_wrt, feed_lower, feed_upper)feed), evaluate=False)

def partially_differentiate_with_respect_to(expression, feed, relation):
    """
    https://docs.sympy.org/latest/tutorial/calculus.html
    """
    return relation(diff(expression.lhs, feed),
                    diff(expression.rhs, feed), evaluate=False)

def X_cross_both_sides_by(expression, feed, relation):
    return relation(cross(feed, expression.lhs),
                    cross(feed, expression.rhs), evaluate=False)

def both_sides_cross_X(expression, feed, relation):
    return relation(cross(expression.lhs, feed),
                    cross(expression.rhs, feed), evaluate=False)

def X_dot_both_sides(expression, feed, relation):
    return relation(dot(feed, expression.lhs),
                    dot(feed, expression.rhs), evaluate=False)

def both_sides_dot_X(expression, feed, relation):
    return relation(dot(expression.lhs, feed),
                    dot(expression.rhs, feed), evaluate=False)

def make_expression_power(expression, feed, relation):
    return relation(Pow(feed, expression.lhs), Pow(feed, expression.rhs), evaluate=False)
...
def add_expression_1_to_expression_2(expression1, expression2, relation):
    return relation(expression1.lhs + expression2.lhs,
                    expression1.rhs + expression2.rhs, evaluate=False)

def substitute_RHS_of_expression_1_into_expression_2(expression1, expression2, relation):
    """
    >>> expr1 = parse_latex('a = g + 2')
    >>> expr2 = parse_latex('a + b = c + d')

    """
    return relation(expression2.lhs.subs(expression1.rhs, expression1.lhs),
                    expression2.rhs.subs(expression1.rhs, expression1.lhs), evaluate=False)

def substitute_LHS_of_expression_1_into_expression_2(expression1, expression2, relation):
    """
    >>> expr1 = parse_latex('a = g + 2')
    >>> expr2 = parse_latex('a + b = c + d')

    """
    return relation(expression2.lhs.subs(expression1.lhs, expression1.rhs),
                    expression2.rhs.subs(expression1.lhs, expression1.rhs), evaluate=False)

def mult_expression_1_by_expression_2(expression1, expression2, relation):
    return relation(Mul(expression1.lhs, expression2.lhs),
                    Mul(expression1.rhs, expression2.rhs), evaluate=False)
...
def raise_both_sides_to_power(expression, feed, relation):
    return relation(Pow(expression.lhs, feed), Pow(expression.rhs, feed), evaluate=False)
