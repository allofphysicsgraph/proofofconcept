...
def create_step(latex_for_step_dict, inf_rule, name_of_derivation, print_debug, path_to_pkl):
    ...
    for which_eq, latex_expr_str in latex_for_step_dict.items():
        if 'input' in which_eq:
            ...
            step_dict['inputs'].append({'expr local ID':create_expr_local_id(print_debug, path_to_pkl),
                                        'expr ID':expr_id})
        elif 'output' in which_eq:
            expr_id = create_expr_id(print_debug, path_to_pkl)
            expressions_for_this_step[expr_id] = latex_expr_str
            step_dict['outputs'].append({'expr local ID':create_expr_local_id(print_debug, path_to_pkl),
                                         'expr ID':expr_id})
        elif 'feed' in which_eq:
            step_dict['feeds'].append({'feed local ID':create_expr_local_id(print_debug, path_to_pkl),
                                       'feed latex':latex_expr_str })
        else:
...
def write_step_to_db(name_of_derivation, step_dict, expr_dict, path_to_pkl):
    ...
    for expr_id, expr_latex in expr_dict.items():
        expressions_dict[expr_id] = expr_latex
    try:
        derivations_dict[name_of_derivation].append(step_dict)
    except KeyError:
        derivations_dict[name_of_derivation] = []
...
def write_step_to_graphviz_file(step_dict, expressions_dict, fil, print_debug, path_to_pkl):
    """
    >>> step_dict = {}
    >>> write_step_to_graphviz_file(step_dict, fil)
    """
...
def create_derivation_png(name_of_derivation, print_debug, path_to_pkl):
    ...
    with open(dot_filename,'w') as fil:
        ...
        for step_dict in derivations_dict[name_of_derivation]:
            write_step_to_graphviz_file(step_dict, expressions_dict, fil, print_debug, path_to_pkl)
