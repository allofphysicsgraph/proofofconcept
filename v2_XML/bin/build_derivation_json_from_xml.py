...
#   print("name_of_set_to_make="+str(name_of_set_to_make))
    ...
    # [(infrule_name,infrule_tunid), (expression_punid,expression_tunid), (feed_tunid),...]
    ary_of_unique_nodes = []
    # [(infrule_tunid,expression_tunid),(infrule_tunid,expression_tunid),...] # pairs are source/target
    ary_of_edges = []
...
#         print(edge_source_target)
            ...
            input_label_array = []
            for statement_counter in range(
                    len(input_nodes.getElementsByTagName('expression_permenant_unique_id'))):
                expression_punid = physgraf.convert_tag_to_content(
                    input_nodes, 'expression_permenant_unique_id', statement_counter)
                expression_tunid = physgraf.convert_tag_to_content(
                    input_nodes, 'expression_temporary_unique_id', statement_counter)
...
#         print(edge_source_target)
        ...
        for output_nodes in connector.getElementsByTagName(
            ...
            for statement_counter in range(
                ...
                expression_punid = physgraf.convert_tag_to_content(
                    output_nodes, 'expression_permenant_unique_id', statement_counter)
                expression_tunid = physgraf.convert_tag_to_content(
                    output_nodes, 'expression_temporary_unique_id', statement_counter)
        ...
        if len(node) == 2:  # list, either infrule or expression
            if node[0].isdigit():  # expression
                output = subprocess.check_output(
                    "file " + expression_pictures_path + "/" + node[0] + ".png", shell=True)
                matchObj = re.match(
                    r'.*data, (\d+) x (\d+), .*', output, re.M | re.I)
...
#         print("  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_expression_png/"+node[0]+".png\", \"width\": "+img_width+", \"height\": "+img_width+", \"label\": \""+node[0]+"|"+node[1]+"\"},")
                node_list_str = node_list_str + "  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_expression_png/" + \
                    node[0] + ".png\", \"width\": " + img_width + ", \"height\": " + img_height + ", \"label\": \"" + node[0] + "|" + node[1] + "\"},\n"
            else:  # infrule
                #         print(node[0])
                output = subprocess.check_output(
                    "file " + infrule_pictures_path + "/" + node[0] + ".png", shell=True)
#         print(output)
                matchObj = re.match(
                    r'.*data, (\d+) x (\d+), .*', output, re.M | re.I)
                img_width = matchObj.group(1)
...
#         print("  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_infrule_png/"+node[0]+".png\", \"width\": "+img_width+", \"height\": "+img_width+", \"label\": \""+node[0]+"|"+node[1]+"\"},")
                node_list_str = node_list_str + "  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_infrule_png/" + \
                    node[0] + ".png\", \"width\": " + img_width + ", \"height\": " + img_height + ", \"label\": \"" + node[0] + "|" + node[1] + "\"},\n"
        else:  # feed
            output = subprocess.check_output(
                "file " + feed_pictures_path + "/" + node + ".png", shell=True)
            matchObj = re.match(
                r'.*data, (\d+) x (\d+), .*',
                output,
                re.M | re.I)
...
#       print("  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_expression_png/"+node+".png\", \"width\": "+img_width+", \"height\": "+img_width+", \"label\": \""+node+"\"},")
            node_list_str = node_list_str + "  {\"img\": \"http://allofphysicsgraph.github.io/proofofconcept/lib/images_feed_png/" + \
                node + ".png\", \"width\": " + img_width + ", \"height\": " + img_height + ", \"label\": \"" + node + "\"},\n"
...
#           target_index=index
    ...
    for edge_pair in ary_of_edge_indices:
        ...
        edge_list_str = edge_list_str + \
            "  {\"source\":" + str(edge_pair[0]) + ",\"target\":" + str(edge_pair[1]) + "},\n"
