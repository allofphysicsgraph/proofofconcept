...
for item in statementsDB.getElementsByTagName('statement'):
  ...
  if ((redraw_statement_pictures_boolean) or ((not redraw_statement_pictures_boolean) and not (os.path.isfile(statement_pictures+'/'+statement_indx+'.'+extension)))): # if file does not exist, redraw
    physgraf.make_picture_from_latex(statement_indx,statement_pictures,statement_latex_math,extension)
...
for item in feedDB.getElementsByTagName('feed'):
  ...
  # http://stackoverflow.com/questions/82831/how-do-i-check-if-a-file-exists-using-python
  if ((redraw_feed_pictures_boolean) or ((not redraw_feed_pictures_boolean) \
      and not (os.path.isfile(feed_pictures+'/'+feed_tunid+'.'+extension)))): # if file does not exist, redraw
    physgraf.make_picture_from_latex(feed_tunid,feed_pictures,feed_latex_math,extension)
